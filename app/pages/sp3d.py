...
with st.sidebar:
    ...
    n_blocks = int(st.number_input("Number of Blocks", min_value=1, step=1, value=15))
    block_size = float(
        st.number_input(
...
if "image" not in st.session_state or reset:
    ...
    blocks = [
        Block(f"block{i}", random_shape(block_size, data_rng), random_color(data_rng))
    ...
    st.session_state["score"] = solver.opt_depth
